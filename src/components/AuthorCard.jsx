...
function AuthorCard({ AuthorsData }) {
  ...
  return (
    <div className="mt-8 justify-center items center flex flex-wrap gap-4 px-6">
      {AuthorsData.map((author) => (
        <div
          key={author.Name} // Use a unique property for the key
          onClick={() => handleAuthorClick(author)}
          className="w-60 p-4 bg-backgroundLight rounded shadow hover:scale-105 flex flex-col justify-center items-center transition transform hover:bg-backgroundLightYellow cursor-pointer flex-wrap sm:w-55"
        >
